  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/audacity") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/audacity",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-audacity",\
                       "backendUrl":"https://ghcr.io/linuxserver/audacity",\
                       "publicUrl":"https://lscr.io/linuxserver/audacity"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-audacity/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-audacity\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-audacity\" \
